    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b shadow-sm" style={{ borderColor: theme.border }}>
      <div className="max-w-6xl mx-auto px-4 h-[60px] flex items-center justify-between gap-4">
        ...
        <Link href="/" className="flex items-center gap-2 shrink-0">
          ...
          <span className="hidden sm:inline-block" style={{ fontSize: '10px', fontWeight: 700, color: theme.accent, background: theme.accentLight, border: `1px solid ${theme.accent}`, padding: '1px 7px', borderRadius: '99px', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
            beta
          </span>
        </Link>
        ...
        <div className="hidden md:flex flex-1 max-w-xs">
          <div className="w-full relative">
            <input type="text" placeholder="Search failures..." readOnly
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <div className="relative flex flex-col items-center gap-0.5 px-3 py-1 rounded-lg"
          ...
          {user ? (
            <div className="relative ml-2">
              <button
                ...
                <span style={{ fontSize: '13px', fontWeight: 500, color: theme.dark, maxWidth: '80px' }} className="truncate hidden sm:block">
      ...
      {/* Mobile menu */}
      {isMobileOpen && (
        <div className="md:hidden border-t bg-white px-4 py-3 flex flex-col gap-1" style={{ borderColor: theme.border }}>
          {navItems.map((item) => (
            <Link key={item.href} href={item.href}

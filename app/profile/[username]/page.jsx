...
import ProfileEditTitle from '@/components/ui/ProfileEditTitle'
import { SEED_PROFILES, SEED_POSTS, getAvatarUrl as getSeedAvatar, formatNumber, getTotalReactions, getFailureScore } from '@/lib/seed-data'
...
const BADGES = [
  ...
  { emoji: '🏆', label: 'Top 10', condition: (score) => score > 10000 },
...
export default async function ProfilePage({ params }) {
  ...
  // Get current session to check if this is the logged-in user's profile
  const { data: { session } } = await supabase.auth.getSession()
  const isOwnProfile = session?.user && dbProfile?.auth_id === session.user.id

  // Fetch real posts if real user, otherwise use seed posts
  let userPosts = []
  if (isRealUser) {
  ...
  const totalScore = isRealUser
    ? userPosts.length * 100 // simple score for real posts
        ...
        <div className="px-5 pb-5">
          <div className="flex items-end justify-between -mt-10 sm:-mt-12 mb-4">
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-4 border-white shadow-lg">
              <img src={avatarUrl} alt={profile.display_name} width={96} height={96} className="w-full h-full object-cover" />
            </div>
            {!isOwnProfile && (
          ...
          <div className="card p-4">
            ...
            <div className="flex flex-col gap-2">
              {[
                ...
                { label: 'Connections', value: formatNum(profile.connections || 0) },
              ].map(row => (
                <div key={row.label} className="flex justify-between text-xs">
                  <span style={{ color: theme.muted }}>{row.label}</span>
                  <span style={{ fontWeight: 700, color: theme.dark, fontFamily: 'var(--font-mono)' }}>{row.value}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          ...
          {userPosts.length > 0 ? (
            ...
            <div className="card p-8 text-center" style={{ color: theme.muted }}>
              ...
              {isOwnProfile && (
                <a href="/submit" className="inline-block mt-3 px-4 py-2 rounded-full text-sm font-semibold text-white"

...
export default function SubmitPage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="card p-10 animate-slide-up">
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.75rem', fontWeight: 700, color: theme.dark, marginBottom: '8px' }}>
            Failure Posted Successfully
          </h2>
          <p style={{ color: theme.muted, marginBottom: '6px', fontSize: '14px' }}>
            Your story is now live. The world is ready to relate.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-6">
            <button onClick={() => { setShowSuccess(false); setContent(''); setCategory('') }}
            ...
            <a href="/"
              className="px-5 py-2.5 rounded-full font-semibold text-sm transition-colors text-center"
              style={{ border: `1.5px solid ${theme.border}`, color: theme.muted }}>
              Back to Feed
            </a>
          </div>
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-6">
        <div style={{ fontSize: '11px', fontWeight: 700, color: theme.muted, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>
          Share with your network
        </div>
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.75rem', fontWeight: 700, color: theme.dark }}>
          Post Your Failure
        </h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="card p-4 flex items-center gap-3">
          <div className="w-11 h-11 rounded-full overflow-hidden shrink-0">
            <img
              src={profile?.avatar_url || `https://api.dicebear.com/8.x/notionists/svg?seed=${profile?.avatar_seed}&backgroundColor=b6e3f4`}
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: theme.muted }}>{profile?.title || 'No headline yet'}</div>
          </div>
        </div>

        {/* Category */}
        <div className="card p-5">
          <h2 style={{ fontSize: '11px', fontWeight: 700, color: theme.muted, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '12px' }}>
            Type of Failure
          </h2>
            ...
            {FAILURE_CATEGORIES.map((cat) => (
              <button key={cat.key} type="button" onClick={() => setCategory(cat.key)}
                ...
                style={category === cat.key
                  ? { background: theme.accent, color: 'white', border: `2px solid ${theme.accent}` }
                  : { border: `2px solid ${theme.border}`, color: theme.muted }
                }>
                {cat.label}
              </button>
            ))}
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            <h2 style={{ fontSize: '11px', fontWeight: 700, color: theme.muted, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              Your Story
            </h2>
            <span style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: isOverLimit ? theme.red : theme.muted, fontWeight: isOverLimit ? 700 : 400 }}>
              {charsLeft}
            </span>
          </div>
          ...
          <textarea
            ...
            style={{ border: `1.5px solid ${isOverLimit ? theme.red : theme.border}`, color: theme.dark }}
          />

          <div className="mt-2 flex items-center gap-2">
            {['📉', '💸', '😭', '🤡', '🚨'].map(emoji => (
              <button key={emoji} type="button" onClick={() => setContent(c => c + emoji)}
        ...
        <p className="text-center" style={{ fontSize: '12px', color: theme.muted }}>

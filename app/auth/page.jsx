...
import { signUpWithEmail, signInWithEmail, signInWithGoogle, signInWithGitHub } from '@/lib/auth'
...
export default function AuthPage() {
  ...
  async function handleEmailSubmit(e) {
    ...
    if (mode === 'signup') {
      ...
      if (error) { setError(error.message); setIsLoading(null); return }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 mb-3">
            ...
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 700, color: theme.dark }}>
              Failbase
            </span>
          </div>
          <h1 style={{ fontSize: '1.1rem', fontWeight: 600, color: theme.dark }}>
            {mode === 'signin' ? 'Welcome back 👋' : 'Join the honest network 📉'}
          </h1>
          <p style={{ fontSize: '13px', color: theme.muted, marginTop: '4px' }}>
            {mode === 'signin'
              ? 'Sign in to post your failures and react to others.'
        ...
        <div className="card p-6">
          ...
          <div className="flex flex-col gap-2 mb-5">
            <button
              ...
              style={{ border: `1.5px solid ${theme.border}`, color: theme.dark, background: 'white' }}>
              {isLoading === 'google' ? '⏳' : (
          ...
          {success ? (
            <div className="text-center py-4">
              ...
              <p style={{ fontSize: '14px', fontWeight: 600, color: theme.dark }}>{success}</p>
            ...
            <form onSubmit={handleEmailSubmit} className="flex flex-col gap-3">
              ...
              {error && (
                <p className="text-sm text-center" style={{ color: theme.red }}>{error}</p>
              )}
              ...
              <button
                ...
                style={{ background: `linear-gradient(135deg, ${theme.accent}, ${theme.highlight})` }}>
                {isLoading === 'email' ? '⏳ Loading...' : mode === 'signin' ? 'Sign In' : 'Create Account'}
              </button>
            </form>
          )}

          {/* Toggle mode */}
          <p className="text-center mt-4" style={{ fontSize: '13px', color: theme.muted }}>
        ...
        <p className="text-center mt-4" style={{ fontSize: '11px', color: theme.muted, opacity: 0.6 }}>
